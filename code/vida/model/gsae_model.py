...
class GSAE(pl.LightningModule):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        # bug fixed
        _, log_losses_gsae = self.loss_multi_GSAE(recon_x=x, x=x_hat,
                                        mu=mu, logvar=logvar,
                                        y_pred=y_hat, y=y,
                                    alpha=self.hparams.alpha, beta=self.hparams.beta,
                                    batch_idx=batch_idx)
        
        log_losses = {'val_loss' : log_losses_gsae["train_loss"],
                    'val_recon_loss' : log_losses_gsae["recon_loss"],
                    'val_pred_loss' : log_losses_gsae["pred_loss"],
                    'val_kl_loss': log_losses_gsae["kl_loss"]}
    ...
    def validation_epoch_end(self, outputs):
        ...
        # self.log('val_loss_epoch_end', avg_loss, prog_bar=True)
