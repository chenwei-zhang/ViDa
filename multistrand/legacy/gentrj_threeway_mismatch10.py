...
def create_setup():
    ...
    bm_design_mm10 = "GGTGAGTCATTTGAGGT"
    incum_extra_design = "TGGTGTTTGTGGGTGT"
    bm_design_parttgt = "GGTGAGTGATTTGAGGT"

    # creating Domain objects
    toehold = Domain(name="toehold", sequence=toehold_seq[0:toehold_length])
    branch_migration_inv = Domain(name="bm", sequence=bm_design_mm10)
    branch_migration_tgt = Domain(name="bm", sequence=bm_design_parttgt)
    

    # invader
    invader = branch_migration_inv + toehold
    invader.name = "invader"

    # target (substrate)
    toehold_extra = Domain(name="toehold_extra", sequence=toehold_seq[toehold_length:]) 
    target = toehold_extra.C + toehold.C + branch_migration_tgt.C
    target.name = "target"

    # incumbent
    incumbent_extra = Domain(name="incumbent_extra", sequence=incum_extra_design)
    incumbent = incumbent_extra+branch_migration_tgt
    ...
    o = Options(
        ...
        gt_enable = False,
        output_interval = 5000, # record every 1000 steps
        verbosity=0,
...
if __name__ == '__main__':
    ...
    for i in range(2):
        ...
        sys.stdout = open(f"./raw_data/Machinek-Mismatch10-og/Machinek-Mismatch10-{i}.txt", "a")
